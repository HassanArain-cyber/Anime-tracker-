...
function renderSugs(box, results, inp, onSelect) {
  ...
  box.querySelectorAll('.sug-item').forEach((el, i) => {
    el.addEventListener('click', () => {
      ...
      if (data[i]) {
        selectedAnime = data[i];
        // Show English title if available, else Japanese
        inp.value = data[i].title_english || data[i].title || '';
...
function handleAdd(e) {
  ...
  if (handleAdd._busy) return;
  handleAdd._busy = true;
  setTimeout(() => { handleAdd._busy = false; }, 800);
  ...
  const displayTitle = selectedAnime?.title_english || selectedAnime?.title || title;

  const entry = {
    id:      Date.now(),
    num:     watchedList.length + 1,
    title:   displayTitle,
    titleJp: selectedAnime?.title || title,
...
function renderWatched() {
  ...
  grid.innerHTML = items.map((a, idx) => {
    const ep = a.ept ? `${a.epw || 0} / ${a.ept} eps` : (a.epw ? `${a.epw} eps` : '');
    const displayTitle = a.title || a.titleJp || 'Unknown';
    ...
    return `<div class="anime-card" style="animation-delay:${idx * 0.04}s">
      <div class="card-inner">
        <div class="card-poster-small">
          ${a.poster
            ? `<img src="${a.poster}" onerror="this.parentElement.innerHTML='<div class=no-poster-sm>🎌</div>'" loading="lazy"/>`
            : `<div class="no-poster-sm">🎌</div>`}
          <div class="card-num-sm">${a.num}</div>
        </div>
        <div class="card-info">
          <div class="card-info-top">
            <span class="card-status ${statusClass(a.status)}">${a.status}</span>
            <button class="card-fav ${a.fav ? 'on' : ''}" onclick="toggleFav(${a.id})">${a.fav ? '❤️' : '🤍'}</button>
          </div>
          <div class="card-title-new">${displayTitle}</div>
          ${genres ? `<div class="card-genre-new">${genres}</div>` : ''}
          ${ep     ? `<div class="card-eps-new">📺 ${ep}</div>` : ''}
          ${a.rating ? `<div class="card-stars-new">${starsHTML(a.rating, 13)}</div>` : ''}
          <div class="card-actions-new">
            ...
            <button class="card-action-btn danger" onclick="deleteAnime(${a.id})">🗑️ Del</button>
          </div>
        </div>
      </div>
    </div>`;
